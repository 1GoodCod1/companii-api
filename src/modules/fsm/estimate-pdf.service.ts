...
export class EstimatePdfService {
  async build(data: EstimatePdfData): Promise<Buffer> {
    ...
    doc.fillColor('#FFFFFF').fontSize(22).font('Helvetica-Bold').text('SMETĂ / DEVIZ', 48, 42);
    ...
    const metaY = 230;
    doc.fillColor('#111827').font('Helvetica-Bold').fontSize(10).text('Detalii proiect', 48, metaY);
    doc.font('Helvetica').fontSize(9).fillColor('#374151');
    ...
    for (const stage of data.stages) {
      doc.fillColor('#065F46').font('Helvetica-Bold').fontSize(10).text(stage.name, 48, rowY);
      rowY += 16;
